...
const checkDeadlineReminders = async () => {
  try {
    ...
    for (const task of tasksNearing1Hour) {
      logger.info(`Processing 1 hour reminder for task: ${task.taskName} to user: ${task.owner.email}`);
    ...
    for (const task of overdueTasks) {
      logger.info(`Processing overdue reminder for task: ${task.taskName} to user: ${task.owner.email}`);
