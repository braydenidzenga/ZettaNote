...
export const addAvatar=async(req,res)=>{
    try{
        ...
        const cloudinaryRes=await cloudinary.uploader.upload(base64Avatar,{ folder:"avatars", public_id:`avatar_${userId}` })
        const avatarUrl=cloudinaryRes.secure_url;
        const user=await userModel.findByIdAndUpdate(userId,{avatar:avatarUrl},{new:true}).select('-password');
